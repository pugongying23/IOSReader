...
@interface XMGPlayingViewController () <UIScrollViewDelegate, AVAudioPlayerDelegate,songerViewdelegate>
...
@property(nonatomic,strong) songerView * songers;
...
- (void)viewDidLoad {
    ...
    self.songers =[[songerView alloc]initWithFrame:CGRectMake(self.view.frame.size.width, 0, self.view.frame.size.width, self.view.frame.size.height)];
    self.songers.delegate=self;
    [self.view addSubview:self.songers];
}
...
- (IBAction)moreMusics:(UIButton *)sender {
    
    [UIView animateWithDuration:1 animations:^{
        self.songers.frame = CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height);
    }];
}
...
-(void)directoryViewHiden{
    ...
    [UIView animateWithDuration:1 animations:^{
        self.songers.frame = CGRectMake(self.view.frame.size.width, 0, self.view.frame.size.width, self.view.frame.size.height);
    }];
}

-(void)chooseCheap:(XMGMusic *)cheap{
    [self playingMusicWithMusic:cheap];
